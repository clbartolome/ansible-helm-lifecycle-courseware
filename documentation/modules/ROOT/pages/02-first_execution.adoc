  ...
  tasks:
    ...
    - name: Upload Helm Base Chart package to Nexus
      ansible.builtin.shell: |
        curl -X 'POST' \
        'http://%USER%:%USER%@nexus.app-lifecycle-lab.svc:8081/service/rest/v1/components?repository=%USER%' \
...
dependencies:
- name: base-chart
  repository: http://nexus.apps.cluster-wmbjp.wmbjp.sandbox2100.opentlc.com/repository/%USER%/
  version: 1.0.0
----	

* Commit your changes.

* Login into *ArgoCD* and open your *demo-app* application. *ArgoCD* automatically syncs each 3 minutes but we can manually force *ArgoCD* synchronization by pressing the *SYNC* button (use the default synchronization values). Now you should see your changes applied in the section *LAST SYNC RESULT*.
